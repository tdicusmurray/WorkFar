...
class Hybrid_Provider_Adapter {
	...
	function login() {
		...
		if (!isset($this->params["login_start"]) ) {
			$this->params["login_start"] = "/work/auth/hauth.start={$this->id}&hauth.time={$this->params["hauth_time"]}";
		}

		# for default HybridAuth endpoint url hauth_login_done_url
		# 	auth.done   required  the IDp ID
		if (!isset($this->params["login_done"]) ) {
			$this->params["login_done"] = "/work/auth/hauth.done={$this->id}";
		}
		...
		if ($this->id=="Live") { 
			$this->params["login_done"] = "/work/auth/live.php"; 
		}
